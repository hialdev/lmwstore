<div>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between mb-3">
        ...
        <div class="mt-3 ms-auto">
            ...
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModalCenter">Tambah Label</a>
        </div>
    </div>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between gap-2">
        <div class="w-100 mb-3">
            <input wire:model="q" type="search" class="form-control" placeholder="Cari Label..">
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Tambah Label</h5>
                ...
                <div class="modal-body">
                    <form action="">
                        <div class="form-group mb-3">
                            <label for="inputNama">Nama Label</label>
                            <input wire:model="name" type="text" class="form-control" id="inputNama">
                            @error('name') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group mb-3">
                            <label for="helpInputTop">Background Color</label>
                            <small class="text-muted"><i>cari icon di <a href="https://icon-sets.iconify.design/" target="_blank">iconify.design</a></i></small>
                            <input wire:model="bg_color" type="color" class="form-control form-control-color" id="helpInputTop" value="{{$bg_color}}">
                            @error('bg_color') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group">
                            <span class="p-1 px-2 text-white" style="background-color: {{$bg_color ? $bg_color : '#212121'}}">{{$name ? $name : 'Label Text'}}</span>
                        </div>
                    </form>
                </div>
    ...
    <div wire:ignore.self class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Edit Label</h5>
                    <button type="button" class="close" data-bs-dismiss="modal"
                        aria-label="Close">
                        <i data-feather="x"></i>
                    </button>
    ...
    <div class="table-responsive bg-white" wire:ignore.self>
        <table class="table mb-0 table-md">
            ...
            <tbody>
                @forelse ($label as $lbl)
                <tr>
                    <td class="fw-bold"><input type="checkbox" wire:model="selected" value="{{$lbl->id}}"></td>
                    <td>{{$lbl->name}}</td>
                    <td><span class="p-2 me-2" style="background-color: {{$lbl->bg_color}}"></span>{{$lbl->bg_color}}</td>
                    <td>{{$lbl->updated_at}}</td>
                    <td>
                        <button wire:click="edit({{$lbl->id}})" class="btn btn-sm btn-primary mb-1" data-bs-toggle="modal" data-bs-target="#editModal">Edit</button>
                        <button wire:click="delSingle({{$lbl->id}})" data-bs-toggle="modal" data-bs-target="#delSingle" class="btn btn-sm btn-danger mb-1">Del</button>
                    </td>
                </tr>
                @empty
                <tr>
